...
export default function ServiceComponent({title, description}: ServiceComponentProps){
    ...
    return <>
        <div className={`bg-black rounded-3xl p-6 items-center flex flex-col gap-8 transition-transform duration-300 ease-in-out ${isHovered ? "bg-[#c5ff41] rotate-1" : ""}`} onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}>
            ...
            <p className={`text-md text-[#bebebe] text-wide ${isHovered ? "text-black" : "text-white"}`}>
                {description}
            </p>
        </div>
    </>
